...
export class AfterValueChangedDirective implements OnDestroy {
  ...
  constructor() {
    this.subscription = this.stream.pipe(debounceTime(this.valueChangeDelay)).subscribe((value: any) => {
  ...
  public onValueChange2(value: any): void {
    this.stream.next(value.target.value);
  }
}
